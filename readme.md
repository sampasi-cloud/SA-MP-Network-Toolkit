...
1. Поместите `bypasser.lua` в папку со скриптами вашего клиента
2. Загрузите скрипт с помощью Lua-загрузчика
3. Используйте команды, описанные в документации скрипта

### Документация по пакетам

Для получения информации о конкретных пакетах откройте файл `samp-packets-nop-effects.md`.

## 🤝 Вклад

Ваш вклад приветствуется! Если у вас есть предложения по улучшению проекта:
